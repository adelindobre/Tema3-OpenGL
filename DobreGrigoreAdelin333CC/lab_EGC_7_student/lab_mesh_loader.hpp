...
namespace lab{
	...
	void bindMesh(Mesh* mesh, unsigned int size_of_vertex){
		...
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, size_of_vertex, (void*)0);					//trimite pozitii pe pipe 0
	}
	...
	void drawMesh(Mesh *mesh, bool filled){
		...
		glUniform3f(glGetUniformLocation(gl_program_shader_gouraud, "color"), mesh->r, mesh->g, mesh->b);
		glBindVertexArray(mesh->vao);
		glDrawElements(GL_TRIANGLES, mesh->count, GL_UNSIGNED_INT, 0);	
	}
	void setColorObject(float r, float g, float b, bool filled){
		if (filled)
			glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
		else
			glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);

		glUniform3f(glGetUniformLocation(gl_program_shader_gouraud, "color"), r, g, b);
	}
